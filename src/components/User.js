...
function User({account}) {
  ...
  useEffect(() => {
    async function fetchRentals() {
      const Rentals = Moralis.Object.extend("RM_New_Bookings");
  ...
  return (
    <>
      ...
      <div style={{display:"flex", justifyContent:"start", flexWrap:"wrap", gap:"10px"}}>
        {userRentals && 
          userRentals.map((e)=>{
            console.log("Dates=" + e.attributes.datesBooked);
            return(
              <div style={{ width:"200px"}}>
                <Card
                  isDisabled
                  title={e.attributes.city}
                  description={`${e.attributes.datesBooked[0]} for ${e.attributes.datesBooked.length} Day(s)`}
                >
                  <div>
                    <img
                      width="180px"
